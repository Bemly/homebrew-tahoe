// updater/zstd.swift —— 与 Formula/zstd.rb 一一同名的检查器入口
//
// 运行：swiftc updater/UpdaterCore.swift updater/zstd.swift -o /tmp/check-zstd && /tmp/check-zstd
...
struct ZstdCheck {
    static func main() {
        runCheck(CheckConfig(formula: "zstd", brewName: "zstd"))
    }
}
