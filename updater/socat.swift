// updater/socat.swift —— 与 Formula/socat.rb 一一同名的检查器入口
//
// 运行：swiftc updater/UpdaterCore.swift updater/socat.swift -o /tmp/check-socat && /tmp/check-socat
...
struct SocatCheck {
    static func main() {
        runCheck(CheckConfig(formula: "socat", brewName: "socat"))
    }
}
