// updater/libssh.swift —— 与 Formula/libssh.rb 一一同名的检查器入口
//
// 运行：swiftc updater/UpdaterCore.swift updater/libssh.swift -o /tmp/check-libssh && /tmp/check-libssh
...
struct LibsshCheck {
    static func main() {
        runCheck(CheckConfig(formula: "libssh", brewName: "libssh"))
    }
}
