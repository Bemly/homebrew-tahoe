class NodeAT24 < Formula
  ...
  bottle do
    ...
    sha256 cellar: :any_skip_relocation, tahoe: "94c4d96205160b3b51896bc7345745231f411931527fcd80c2256531c34823c7"
  end
  ...
  def pre_install
    ...
    ohai "安装 node@24 #{version}（Intel x86_64）"
  end
  ...
  def post_install
    ...
    ohai "node@24 #{version} 安装完成（x86_64 原生）"
  end

  def caveats
    <<~EOS
      node@24 已安装为 Intel(x86_64) 原生二进制，直接取自 Node.js 官方发布包。

      与 homebrew/core 的 node@24 同名，二者不能同时安装（brew 会直接拒绝）。
      切换来源必须先卸载再装另一个：
        用本 tap：brew uninstall node@24 && brew install bemly/tahoe-intel/node@24
        用 core ：brew uninstall bemly/tahoe-intel/node@24 && brew install node@24
